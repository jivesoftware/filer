...
public class MapStoreTest {
    ...
    public void testPerformance() throws Exception {
        ...
        Random r = new Random();
        long filerSize = MapStore.INSTANCE.computeFilerSize(cardinality, 4, false, 4, false);
        ByteBufferBackedFiler filer = new ByteBufferBackedFiler(ByteBuffer.allocate((int) filerSize));
    ...
    public void addRemove() throws IOException {
        ...
        long filerSize = MapStore.INSTANCE.computeFilerSize(4, 1, false, 1, false);
        ByteBufferBackedFiler filer = new ByteBufferBackedFiler(ByteBuffer.allocate((int) filerSize));
        ...
        long filerSize2 = MapStore.INSTANCE.computeFilerSize(4, 1, false, 1, false);
        Filer filer2 = new ByteBufferBackedFiler(ByteBuffer.allocate((int) filerSize2));
    ...
    public void copy() throws IOException, InterruptedException {
        StackBuffer stackBuffer = new StackBuffer();
        long filerSize = MapStore.INSTANCE.computeFilerSize(2, 1, false, 1, false);
        Filer filer = new ByteBufferBackedFiler(ByteBuffer.allocate((int) filerSize));
        ...
        for (int i = 0; i < 128; i++) {
            if (MapStore.INSTANCE.isFull(context)) {
                ...
                Filer newFiler = new ByteBufferBackedFiler(ByteBuffer.allocate((int) filerSize));
    ...
    private static Boolean test(final int _iterations,
        ...
        long filerSize = mapStore.computeFilerSize(_maxSize, keySize, false, payloadSize, false);
