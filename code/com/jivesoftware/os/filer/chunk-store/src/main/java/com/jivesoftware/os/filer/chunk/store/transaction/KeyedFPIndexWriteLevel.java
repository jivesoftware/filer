 ...
 * @param <K>
 */
public class KeyedFPIndexWriteLevel<K> implements LevelProvider<ChunkStore, KeyedFPIndex<K>, ChunkStore, K, PowerKeyedFPIndex> {
    ...
    public <R> R enter(final ChunkStore chunkStore,
        ...
        return parentStore.commit(chunkStore, key, null, KeyedFPIndexCreator.DEFAULT, KeyedFPIndexOpener.DEFAULT, null,
            new ChunkTransaction<PowerKeyedFPIndex, R>() {

                @Override
                public R commit(PowerKeyedFPIndex monkey, ChunkFiler filer) throws IOException {
                    R result = storeTransaction.commit(chunkStore, monkey);
                    return result;
                }
            });
